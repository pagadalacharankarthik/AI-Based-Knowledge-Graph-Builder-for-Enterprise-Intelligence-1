...
def load_vector_db(csv_path=None):
    ...
    print(f"Loading FAISS Vector DB from {csv_path}...")
    try:
    ...
    # Extract naive entities to build normalized search string
    def extract_entities(text):
        words = str(text).split()
        ents = [w for w in words if w.istitle()]
        return list(set(ents[:5]))
        
    _df["entities"] = _df["clean_message"].apply(extract_entities)
    _df["normalized_entities"] = _df["entities"].apply(lambda e: e if isinstance(e, list) else (e.split(",") if isinstance(e, str) else []))
    _df["search_text"] = _df["clean_message"].astype(str) + " " + _df["normalized_entities"].astype(str)
    ...
    embeddings = _model.encode(documents, batch_size=64, show_progress_bar=False)
...
def answer_question(question):
    ...
    api_key = os.environ.get("LLM_API_KEY", "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")
    try:
